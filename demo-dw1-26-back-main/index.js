...
app.patch(
  "/deplacement-image/:id/categorie/:idCategorie",
  [jwtInterceptor, imageOwnerInterceptor],
  (req, res) => {
    const nouvelIdCategorie = parseInt(req.params.idCategorie, 10);

    if (isNaN(nouvelIdCategorie)) {
      return res.status(400).send();
    }

    //on verifie que la nouvelle catégorie appartient bien à l'utilisateur connecté
    connection.query(
      `SELECT id FROM categorie WHERE id = ? AND user_id = ?`,
      [nouvelIdCategorie, req.user.id],
      (erreur, resultat) => {
        if (erreur) {
          console.log(erreur);
          return res.status(500).send();
